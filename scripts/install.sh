...
# install GsDevKit_upgrade into the solo stone
projectSpecUrl="file:$GS_HOME/shared/repos/GsDevKit_upgrade/rowan/specs/GsDevKit_upgrade.ston"
$GS_HOME/shared/repos/stash/scripts/rowan.st --install="$projectSpecUrl" -- $stoneName -l

# create a solo extent and shut down the stone ... only the extent is needed
$GS_HOME/shared/repos/stash/scripts/snapshot.st solo.dbf -- $stoneName -l
stopStone -b $stoneName

echo "Solo script extent is ready. Define the follow env vars for easy access to extent from scripts:"
